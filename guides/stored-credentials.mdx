...
On every subsequent charge — a renewal, an installment, an unscheduled top-up — pass the vault token plus the stored-credential fields and the network references from the original CIT.
...
<CardGroup cols={2}>
  ...
  <Card icon="bolt" title="Unscheduled">
    Use `"type": "unscheduled"` for charges that are authorized in advance but triggered by a merchant-defined event — usage-based billing, account top-ups, or convenience fees.
  </Card>
