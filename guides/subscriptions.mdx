...
Pass an `idempotencyKey` if your reporting job might retry — a duplicate `(meterCode, idempotencyKey)` returns the original event. An optional `occurredAt` sets which billing period the event counts toward (it defaults to now).
