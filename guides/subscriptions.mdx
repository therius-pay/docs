...
Enrolling a customer always requires the customer to be actively present during checkout. Collect a fresh card nonce from the Therius JS SDK first — see the [Browser Checkout](/guides/browser-checkout) guide — then submit it with the subscription request.
...
<Info>
  Always use a fresh SDK nonce when creating a subscription or updating the payment method. Card scheme rules require the customer to be actively present when their card is first saved for future charges. You cannot reuse a nonce from a previous session.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card icon="credit-card" title="Update Payment Method">
    ...
    Replaces the stored card with a new one. Requires a fresh nonce collected while the customer is present. This is also the only way to reactivate a `suspended` subscription.
  </Card>
...
Once all retries are exhausted, the subscription moves to `suspended` and Therius fires a `subscription.suspended` webhook. To reactivate a suspended subscription, the customer must provide a new payment method:
