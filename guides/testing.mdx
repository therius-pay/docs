...
<Steps>
  ...
  <Step title="Resume the payment">
    Call `POST /payment/resume` with `{ "sessionId": "..." }` and your `Authorization: Bearer sk_sandbox_...` header — the `sk_sandbox_` prefix routes to the sandbox. Sessions are single-use and expire after 15 minutes. A successful resume returns the final `PaymentResponse` with `status: "captured"`.
  </Step>
</Steps>
