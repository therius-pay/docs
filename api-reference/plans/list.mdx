---
title: "List All Subscription Plans â€” GET /subscription/plan"
sidebarTitle: "List Plans"
description: "GET /subscription/plan â€” List all billing plans for your merchant account, with optional country filter. Includes subscriber count and archive state."
openapi: "GET /subscription/plan"
---

`GET /subscription/plan` returns all billing plans for your merchant account, including the current subscriber count and archive status for each plan. Use the `country` filter to see only plans available in a specific market â€” global plans (those without country restrictions) are always included in filtered results.
