---
title: "POST /payment/resume — Continue After 3DS Challenge"
sidebarTitle: "Resume Payment"
description: "POST /payment/resume — Resume a payment paused for a 3DS challenge. Pass the sessionId from the pending_3ds response. No API key required."
...
<ParamField body="key" type="string">
  Your merchant private API key. Optional — only used to select the sandbox vs. production environment when the environment cannot be inferred from the `sessionId` prefix.
</ParamField>
...
<ResponseField name="status" type="string">
  ...
  - `"captured"` — payment succeeded and funds are settled (for purchase flows).
  - `"authorized"` — authorization succeeded (for authorization flows).
  - `"declined"` — the payment was declined post-challenge. Check `refusalCode`.
