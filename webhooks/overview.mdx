...
<Note>
  Webhooks are the source of truth for asynchronous outcomes. For voucher and bank-transfer methods especially, do not fulfil an order on the initial `pending` / `pending_action` response — wait for the `payment.captured` webhook.
</Note>
