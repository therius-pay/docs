...
<Warning>
  Nunca llames a `POST /sdk/session` desde el navegador. Requiere tu clave de API privada (`sk_live_...`). Exponer esa clave del lado del cliente permitiría a cualquiera crear sesiones y hacer cobros contra tu cuenta. Haz siempre esta llamada solo desde tu backend.
</Warning>
