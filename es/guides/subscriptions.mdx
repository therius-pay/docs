...
Pasa un `idempotencyKey` si tu trabajo de reporte puede reintentar — un `(meterCode, idempotencyKey)` duplicado devuelve el evento original. Un `occurredAt` opcional fija a qué período de facturación cuenta el evento (por defecto, ahora).
