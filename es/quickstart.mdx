...
El sandbox de Therius es un entorno de pruebas totalmente aislado — las solicitudes llegan a un stub de proveedor de prueba, no se mueve dinero real y no interviene ninguna red de tarjetas. Tu clave de sandbox comienza con `sk_sandbox_...` y toda cuenta de Therius incluye una por defecto. Todo lo que construyas aquí funciona de forma idéntica en producción; solo cambias la clave y la URL base cuando estés listo para salir a producción.
...
<Steps>
  <Step title="Obtén una clave de sandbox">
    Inicia sesión en tu panel de Therius y copia la clave etiquetada como **Sandbox Secret Key**. Se ve así: `sk_sandbox_xxxxxxxxxxxx`.

    Toda solicitud a `https://api-sandbox.therius.io/v1` envía esta clave como `Authorization: Bearer sk_sandbox_...`. El prefijo de la clave (`sk_sandbox_`) le indica a Therius que enrute la solicitud al entorno de sandbox automáticamente — sin configuración adicional.
  </Step>

  <Step title="Realiza tu primera compra">
    Pega el siguiente comando en tu terminal, reemplazando `sk_sandbox_your_key_here` por tu clave de sandbox real.
  ...
  <Step title="Sal a producción">
    ...
    1. Reemplaza `sk_sandbox_...` por tu clave de producción `sk_live_...`.
