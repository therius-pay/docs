...
|---|---|---|---|
| `sk_live_xxx` | Clave privada | Tu servidor | Producción |
| `sk_sandbox_xxx` | Clave privada | Tu servidor | Sandbox |
| `pub_live_xxx` | Clave pública | SDK de JS (JWT de token de cliente) | Producción |
| `pub_sandbox_xxx` | Clave pública | SDK de JS (JWT de token de cliente) | Sandbox |

**Claves privadas** (`sk_live_xxx`, `sk_sandbox_xxx`) autentican todos los endpoints de pago. Mantenlas solo en tu servidor — en variables de entorno, no en el código fuente.

**Claves públicas** (`pub_live_xxx`, `pub_sandbox_xxx`) se incrustan dentro del JWT de token de cliente de corta duración que tu servidor genera y pasa al navegador. El navegador nunca ve una clave privada sin cifrar.
...
<Note>
  El prefijo de la clave determina el entorno automáticamente. Una clave que empieza por `sk_sandbox_` siempre se enruta al sandbox — no necesitas un indicador de entorno aparte ni una ruta de código diferente.
</Note>
...
<Warning>
  Nunca incrustes una clave privada (`sk_live_xxx` o `sk_sandbox_xxx`) en JavaScript de frontend, en el binario de una app móvil ni en un repositorio público. Trata las claves privadas igual que las contraseñas de base de datos.
</Warning>
