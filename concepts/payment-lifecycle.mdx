...
Use `POST /payment/purchase` when you can fulfil the order immediately — digital downloads, SaaS subscriptions, and any product you deliver the moment payment completes. A single round-trip reserves the funds and settles them in one go. The response carries `status: "captured"` and an `id` — the payment handle.
...
|---|---|---|
| `id` | Therius (returned on authorize/purchase) | **The handle for the payment.** Pass it as the `{id}` path segment of capture, refund, cancel and cancel_or_refund. |
