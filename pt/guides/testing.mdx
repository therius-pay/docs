...
O sandbox do Therius (`https://api-sandbox.therius.io/v1`, chaves `sk_sandbox_...`) é um ambiente totalmente isolado. As requisições são tratadas pelo **provedor de sandbox do Therius** integrado, que simula respostas sem tocar em uma rede de cartão — nenhum dinheiro real se move. Esta página lista os dados de teste que impulsionam cada resultado simulado.
...
- Envie a requisição com uma chave `sk_sandbox_...`, ou adicione o cabeçalho `X-Environment: sandbox`.
...
<Steps>
  ...
  <Step title="Retome o pagamento">
    Chame `POST /payment/resume` com `{ "sessionId": "..." }` e o seu cabeçalho `Authorization: Bearer sk_sandbox_...` — o prefixo `sk_sandbox_` roteia para o sandbox. As sessões são de uso único e expiram após 15 minutos. Um resume bem-sucedido devolve o `PaymentResponse` final com `status: "captured"`.
  </Step>
</Steps>
