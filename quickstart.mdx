...
<Steps>
  <Step title="Get a sandbox key">
    ...
    Every request to `https://api-sandbox.therius.io/v1` sends this key as `Authorization: Bearer sk_sandbox_...`. The key prefix (`sk_sandbox_`) tells Therius to route the request to the sandbox environment automatically — no extra configuration needed.
  </Step>
