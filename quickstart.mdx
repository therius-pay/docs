...
<Steps>
  <Step title="Get a sandbox key">
    Log in to your Therius dashboard and copy the key labeled **Sandbox Secret Key**. It looks like `sk_sandbox_xxxxxxxxxxxx`.

    Every request to `https://api-sandbox.therius.io/v1` must include this key either in the `Authorization` header or in the `key` field of the request body. The key prefix (`sk_sandbox_`) tells Therius to route the request to the sandbox environment automatically — no extra configuration needed.
  </Step>

  <Step title="Fire your first purchase">
    Paste the following command into your terminal, replacing `sk_sandbox_your_key_here` with your actual sandbox key.
    ...
    Save the `paymentCode` — it is Therius's receipt ID for this payment and you will need it for refunds, cancellations, and lookups.
  </Step>

  <Step title="Look up the payment">
    Retrieve any payment by passing its `paymentCode` (Therius's receipt ID, returned in the purchase response) to `GET /payment/inquiry/{id}`.

    ```bash
    curl https://api-sandbox.therius.io/v1/payment/inquiry/PAY-abc123xyz \
  ...
  <Step title="Go live">
    ...
    1. Replace `sk_sandbox_...` with your production key `sk_live_...`.
