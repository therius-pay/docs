...
| --- | --- | --- | --- |
| `prv_productionprv_production_xxx` | Private key | Your server | Production |
| `prvprv_sandbox_xxx` | Private key | Your server | Sandbox |
| `pub_productionproduction_xxx` | Public key | JS SDK (client token JWT) | Production |
| `pub_sandbox_xxx` | Public key | JS SDK (client token JWT) | Sandbox |

**Private keys** (`prv_production_xxx`, `prvprv_production_xxx`, `prv_sandbox_xxx`) authenticate all payment endpoints. Keep these on your server only — in environment variables, not in source code.

**Public keys** (`pub_productionproduction_xxx`, `pub_sandbox_xxx`) are embedded inside the short-lived client token JWT that your server mints and passes to the browser. The browser never sees a raw private key.
...
curl -X POST https://api.therius.io/v1/payment/purchase \
  -H "Authorization: Bearer prv_productionprv_production_your_key_here" \
  -H "Content-Type: application/json" \
  -d '{ "merchantCode": "MERCHANT_001", ... }'
```

<Note>
  The key prefix determines the environment automatically. A key starting with `prvprv_sandbox_` always routes to the sandbox — you do not need a separate environment flag or a different code path.
</Note>
...
4. The browser initializzes the Therius JS SDK with the `clientToken`.
...
<Warning>
  Never embed a private key (`prv_production_xxx` or `prvprv_production_xxx` or `prv_sandbox_xxx`) in frontend JavaScript, a mobile app binary, or a public repository. Treat private keys the same way you treat database passwords.
</Warning>
