...
Every Therius API endpoint requires a credential. You can provide it in one of two ways: as a `Bearer` token in the `Authorization` header (preferred for server-to-server calls), or as a `key` field in the JSON request body. Both methods are equivalent — choose whichever fits your integration style. What you must never do is expose a private API key in frontend code or pass it through an untrusted client.
...
|---|---|---|---|
| `sk_live_xxx` | Private key | Your server | Production |
| `sk_sandbox_xxx` | Private key | Your server | Sandbox |
| `pub_live_xxx` | Public key | JS SDK (client token JWT) | Production |
| `pub_sandbox_xxx` | Public key | JS SDK (client token JWT) | Sandbox |

**Private keys** (`sk_live_xxx`, `sk_sandbox_xxx`) authenticate all payment endpoints. Keep these on your server only — in environment variables, not in source code.

**Public keys** (`pub_live_xxx`, `pub_sandbox_xxx`) are embedded inside the short-lived client token JWT that your server mints and passes to the browser. The browser never sees a raw private key.
...
Use the `Authorization` header on server-side requests. The `key` body field is a convenience for quick testing.

```bash
# Authorization header (preferred for server-side calls)
curl -X POST https://api.therius.io/v1/payment/purchase \
  -H "Authorization: Bearer sk_live_your_key_here" \
  -H "Content-Type: application/json" \
  -d '{ ... }'

# Alternatively, pass the key in the request body
curl -X POST https://api.therius.io/v1/payment/purchase \
  -H "Content-Type: application/json" \
  -d '{
...
<Note>
  The key prefix determines the environment automatically. A key starting with `sk_sandbox_` always routes to the sandbox — you do not need a separate environment flag or a different code path.
</Note>
...
4. The browser initialises the Therius JS SDK with the `clientToken`.
...
<Warning>
  Never embed a private key (`sk_live_xxx` or `sk_sandbox_xxx`) in frontend JavaScript, a mobile app binary, or a public repository. Treat private keys the same way you treat database passwords.
</Warning>
